...
void drawShellInfo(char *path) {
	// Title
	char version[8];
	sprintf(version, "%X.%02X", VITATOOLBOX_VERSION_MAJOR, VITATOOLBOX_VERSION_MINOR);
	if (version[3] == '0')
		version[3] = '\0';

	pgf_draw_textf(SHELL_MARGIN_X, SHELL_MARGIN_Y, TITLE_COLOR, FONT_SIZE, "%s %s", is_molecular_shell ? "molecularShell" : "VitaToolBox", version);
	// Battery	
	...
	float battery_percent_x = ALIGN_RIGHT(SCREEN_WIDTH - 8, vita2d_pgf_text_width(font, FONT_SIZE, batteryPercent));
	pgf_draw_text(battery_percent_x, 16, DATE_TIME_COLOR, 0.7, batteryPercent);*/
...
void initUsb() {
	...
	usbdevice_modid = startUsb("ux0:VitaToolBox/module/usbdevice.skprx", path, SCE_USBSTOR_VSTOR_TYPE_FAT);
...
int dialogSteps() {
	...
	switch (dialog_step) {
		...
		case DIALOG_STEP_PLUGINSMANAGER_AGREEMENT:
			if (msg_result == MESSAGE_DIALOG_RESULT_YES) {
				pluginsmanagerAgree();
				dialog_step = DIALOG_STEP_NONE;
			} else if (msg_result == MESSAGE_DIALOG_RESULT_NO) {
				pluginsmanagerDisagree();
				dialog_step = DIALOG_STEP_NONE;
			} else if (msg_result == MESSAGE_DIALOG_RESULT_FINISHED) {
				pluginsmanagerAgree();
				dialog_step = DIALOG_STEP_NONE;
		...
		case DIALOG_STEP_DATABASE_REBUILD_QUESTION:
			if (msg_result == MESSAGE_DIALOG_RESULT_YES) {
...
int fileBrowserMenuCtrl() {
	...
	// Settings menu
	if (current_buttons & SCE_CTRL_LTRIGGER && pressed_buttons & SCE_CTRL_START) {
		openSettingsMenu();
	} else if (pressed_buttons & SCE_CTRL_RIGHT_ANALOG_UP) {
		openPluginsManagerMenu();
	} else if (pressed_buttons & SCE_CTRL_RIGHT_ANALOG_LEFT) {
		openDataBaseMenu();
	} else if (pressed_buttons & SCE_CTRL_RIGHT_ANALOG_RIGHT) {
		openVitaidentMenu();
	}

	// Plugins Manager & DataBase menu
	if (dir_level == 0) {
	
		if (pressed_buttons & SCE_CTRL_TRIANGLE) {
		//stringCredit = strlen(CREDITS);
		initMessageDialog(SCE_MSG_DIALOG_BUTTON_TYPE_OK, CREDITS, 1);
		dialog_step = DIALOG_STEP_SYSTEM;
		}/*else if (current_buttons & SCE_CTRL_LTRIGGER && pressed_buttons & SCE_CTRL_RIGHT_ANALOG_DOWN) {
			launchActivator();
		}*/	
		/*if (pressed_buttons & SCE_CTRL_RIGHT_ANALOG_UP) {
			openPluginsManagerMenu();
		} else if (pressed_buttons & SCE_CTRL_RIGHT_ANALOG_LEFT) {
			openDataBaseMenu();
...
int shellMain() {
	...
	// Init plugins manager menu
	initPluginsManagerMenu();
	...
	while (1) {
		...
		if (dialog_step != DIALOG_STEP_NONE) {
			...
			if (getSettingsMenuStatus() != SETTINGS_MENU_CLOSED) {
				settingsMenuCtrl();
			} else if (getPluginsManagerMenuStatus() != PLUGINSMANAGER_MENU_CLOSED) {
				pluginsmanagerMenuCtrl();
...
int main(int argc, const char *argv[]) {
	...
	// Load pluginsmanager settings
	//loadPluginsManagerConfig();
