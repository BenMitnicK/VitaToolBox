...
/*int get_Decimal(char *str) {
	//char * endPtr;
	//char st_r[100] = {0};
	//strcpy(st_r, str);
	//int az = strtod(str, &endPtr);
	//double ret;
	//ret = strtod(st_r, NULL);
	//char az[MAX_PATH_LENGTH];
	//snprintf(az, MAX_PATH_LENGTH, "%.2f", atof(str));
	//float q = atof(az);
	//double ret = strtod(az, NULL);
	//debugPrintf("The number(double) is %f   %d   %lf\n", q, ret, ret);
	//debugPrintf("VALUE DECIMAL1: %s  %d\n", az, strtod(az, NULL));
	return atof(str);
}*/
	  ...
	  //float param = atof(string);
		if (strcasecmp(name, entries[i].name) == 0) {
			switch (entries[i].type) {
				...
				/*case CONFIG_TYPE2_DECIMAL:
					//param = atof(string);
					*(uint32_t *)entries[i].value = get_Decimal(string);
					 //debugPrintf("VALUE DECIMAL1: %d\n", param);
					 debugPrintf("VALUE DECIMAL2: %d\n", *(uint32_t *)entries[i].value);
					break;*/
